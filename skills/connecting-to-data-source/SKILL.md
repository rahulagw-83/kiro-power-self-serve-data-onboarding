...
Register an external data source with AWS Glue so downstream pipelines can move data from it. A Glue connection stores the network config, driver, and credential reference for one source. Create once per source, reuse across jobs.
...
This skill handles **Step 4 (Set Up Connection)** and **Step 5 (Test Connectivity)** of the master onboarding workflow defined in `steering/onboarding-workflow.md`. Once this skill confirms the connection is ready, control returns to the onboarding workflow at Step 6 (Infer Schema).

**Naming convention alignment:** All connections created by this skill MUST follow the power's naming convention: `{env}-{engine}-{system}[-{qualifier}]` (e.g., `prod-oracle-hr`, `dev-snowflake-analytics`).

**Source Registry integration:** After successful connection creation and testing, register the connection in the DynamoDB Source Registry with `status=pending` and the `connection_name` attribute populated.
...
If the user names DynamoDB or a local file, stop and tell them: DynamoDB is read directly by Glue without a connection, and local files belong in the S3 file ingestion workflow (`steering/s3-file-ingestion.md`).

If the user names a SaaS source (Salesforce, HubSpot, Workday, SAP), redirect to `steering/saas-streaming-ingestion.md` — those use OAuth2/API-key credentials in Secrets Manager, not Glue Connection objects.
...
- Desired connection name (must follow: `{env}-{engine}-{system}[-{qualifier}]`)
...
- Redshift Serverless: `aws redshift-serverless list-workgroups`
...
1. IAM database authentication (best — no long-lived passwords)
2. AWS Secrets Manager with rotation enabled
3. AWS Secrets Manager without rotation
4. Plaintext in connection properties (NEVER for production)
...
On success in both phases, tell user the connection name is ready for pipeline generation. On failure in either phase, Step 8.
...
| S3 path (not a connection source) | Redirect to `steering/s3-file-ingestion.md` |
...
- Connection names are immutable. Choose carefully following the `{env}-{engine}-{system}` convention.
