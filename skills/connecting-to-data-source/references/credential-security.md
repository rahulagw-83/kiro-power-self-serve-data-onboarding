...
**Naming convention:** Secrets MUST follow `data-platform/{env}/{connection-name}` or `glue/{connection-name}/credentials` pattern for consistency across the platform.

JDBC sources:

```bash
aws secretsmanager create-secret \
  --name "data-platform/prod/oracle-hr" \
...
```bash
aws secretsmanager create-secret \
  --name "data-platform/prod/snowflake-analytics" \
...
aws secretsmanager create-secret \
  --name "data-platform/prod/bigquery-project-id" \
...
```json
{
  ...
  "Resource": "arn:aws:secretsmanager:<region>:<account>:secret:data-platform/*"
}
```
...
"ConnectionProperties": {
  "JDBC_CONNECTION_URL": "...",
  "SECRET_ID": "data-platform/prod/oracle-hr"
}
```
...
**Platform rule:** Plaintext credentials are NEVER acceptable for production connections in this platform. The `pre-ingestion-validation` hook will flag any connection not backed by Secrets Manager or IAM auth.
