...
aws glue get-connections --region <REGION> \
  --query 'ConnectionList[].{Name:Name,Type:ConnectionType,LastUpdated:LastUpdatedTimestamp}'
...
After discovery, check the DynamoDB Source Registry to see if any discovered sources are already onboarded:

```bash
aws dynamodb scan --table-name SourceRegistry \
  --filter-expression "source_type = :t" \
  --expression-attribute-values '{":t": {"S": "jdbc"}}' \
  --projection-expression "source_name, connection_name, #s" \
  --expression-attribute-names '{"#s": "status"}'
```

This prevents duplicate onboarding and helps the user understand which sources are already active.
