...
**Naming convention:** Connection names MUST follow `{env}-{engine}-{system}[-{qualifier}]` per the power's naming standard (e.g., `prod-oracle-hr`, `dev-postgres-orders`).
...
**IAM-based (preferred for human/role users):** The Glue role needs `redshift:GetClusterCredentials` on the cluster; no Secrets Manager secret required.
...
- Create a DB user: `CREATE USER etl_user IDENTIFIED WITH AWSAuthenticationPlugin AS 'RDS'` (MySQL) or `GRANT rds_iam TO etl_user` (PostgreSQL)
