...
|---|---|---|
| User + password | Quick start, non-production | `snowflakeUser`, `snowflakePassword` |
| Key-pair (RSA) | Production, long-lived workloads | `snowflakeUser`, `private_key` (PEM, base64) |
| OAuth 2.0 | Enterprise SSO, credential-free for end users | `client_id`, `client_secret`, `refresh_token`, token URL |

OAuth 2.0 for Glue Snowflake connections was released April 2026. For current Snowflake OAuth setup steps, refer to [Snowflake's OAuth docs](https://docs.snowflake.com/en/user-guide/oauth-intro).
...
**Naming convention:** Follow `{env}-snowflake-{system}[-{qualifier}]` (e.g., `prod-snowflake-analytics`, `dev-snowflake-marketing`).
